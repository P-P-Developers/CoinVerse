db.createView("open_position", "mainorders", [
  {
    $lookup: {
      from: "companies",
      pipeline: [
          ...
          branches: [
            {
              case: {
                $and: [
                  {
                    $in: ["$signal_type", ["buy_sell", "sell_buy"]],
                  },
                  {
                    $cond: {
                      if: { $eq: ["$signal_type", "buy_sell"] },
                      then: {
                        $lte: [
                          { $toDouble: "$livePriceData.Ask_Price" },
                          {
                            $ifNull: [
                              {
                                $cond: {
                        ...
                        $gte: [
                          { $toDouble: "$livePriceData.Bid_Price" },
                          {
                            $ifNull: [
                              {
                                $cond: {
          ...
          branches: [
            {
              case: {
                $and: [
                  ...
                  {
                    $regexMatch: {
                      input: "$stoploss_price",
                      regex: /^[0-9]+(\.[0-9]+)?$/,
                    },
                  }, // Check valid numeric format
      ...
      pipeline: [
        {
          $match: {
            $expr: {
              $eq: [{ $toLower: "$ticker" }, { $toLower: "$$orderSymbol" }],
            },
          },
        },
      ],
